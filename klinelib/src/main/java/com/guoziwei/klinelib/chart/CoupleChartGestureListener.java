...
//        Log.d(TAG, "getHighestVisibleX  " +srcChart.getHighestVisibleX());
        ...
        performLoadMore("onChartTranslate");
        syncCharts();
    }

    private void performLoadMore(String TAG) {
        // 加载更多
        Log.d("gzww","performLoadMore 11  "+TAG+"  srcChart.getLowestVisibleX()="+srcChart.getLowestVisibleX());
        if (mOnLoadMoreListener != null && !isLoadMore) {
            Log.d("gzww","performLoadMore 22  "+TAG);
            if (srcChart.getLowestVisibleX() <= 0) {
                Log.d("gzww","performLoadMore 33  "+TAG+" *******************************");
                isLoadMore = true;
                mOnLoadMoreListener.onLoadMore();


                //srcChart.fitScreen();
                srcChart.calculateOffsets();
                srcChart.postInvalidate();

                BarLineChartTouchListener mChartTouchListener= (BarLineChartTouchListener) srcChart.mChartTouchListener;
                mChartTouchListener.stopDeceleration();
            }
        }
    }
